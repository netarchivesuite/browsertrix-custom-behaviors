class QueueIssuuIframe {
  ...
  static PREFIX   = "https://issuu.com/rd4";
  ...
  async awaitPageLoad(ctx) {
    ...
    await waitUntilNode(`//iframe[@id='${QueueIssuuIframe.TARGET_ID}']`);

    // 2) wait for a stable, prefixed src
    let url = null;
    try {
      url = await QueueIssuuIframe.waitForStableIframeSrc({
        id: QueueIssuuIframe.TARGET_ID,
        prefix: QueueIssuuIframe.PREFIX,
        timeoutMs: QueueIssuuIframe.TIMEOUT,
        stableMs: QueueIssuuIframe.STABLE,
        pollMs: QueueIssuuIframe.POLL,
      });
    } catch (e) {
